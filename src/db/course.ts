...
export async function getAllCourseAndContentHierarchy(): Promise<
    ...
    >{
        ...
        const courses = await db.content.findMay({
            where: {
                type: 'video',
                hidden: false
            }
...
export const getFullCourseContent = async (
        ...
        contents.map((content: any) => [
            content.id,
            {
                ...
                videoProgress: content.type === 'video' ? {
                    duration: videoProgress.find((x) => x.contentId === content.id)?.currentTimestamp,
                    markAsCompleted: videoProgress.find((x) => x.contentId === content.id)?.markAsCompleted, videoFullDuration: content.VideoMetadata?.duration
                } : null
            }
        ])
    );
    ...
    contents.sort((a, b) => (a.id < b.id ? -1 : 1)).forEach((content: any) => {
        if(content.parentId){
