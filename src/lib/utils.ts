import { TabType, QueryPaams } from "../actions/types";
...
export const generateCompleteSegments = (
    ...
    userSegments.forEach((segment, index) => {
        ...
        if(index < userSegments.length - 1 && lastEnd < userSegments.[index + 1].start){
            completeSegments.push({
                start: lastEnd,
...
export const createSegmentMarkerElements = (
    ...
    const segmentEnd = isFinite(segment.end) ? (segment.end / (player.duration() || lastSegment.end)) * 100 : 100;
...
export const createSegmentMarkers = (player: any, allSegments: Segment[]) => {
    ...
    seekBar.querySelectorAll('.segment-marker, .segment-tooltip').forEach((el: HTMLElement) => el.remove());
...
export const createSegmentMarkersWithoutDuration = (
    ...
    seekBar.querySelectorAll('.segment-marker, .segment-tooltip').forEach((el: any) => el.remove());
...
export const getUpdateUrl = (
    ...
    const updateQuery = { ...prevQueryParams, ...newQueryParams };
    const queryString = new URLSearchParams(
        updateQuery as Record<string, string>
    ).toString();
...
export const constructCommentPrismaQuery = (
    ...
    let orderBy: Prisma.Emurable<Prisma.CommentOrderByWithRelationInput> = {};
    switch(tabtype){
        case TabType.mu:
...
export const generateHandle = (title: string): string => {
    ...
    const handle = normalizedTitle.toLowerCase().replace(/[^a-z0-9]=/g, '-').replace(/(^-|-$)/g, '').replace(/[^\x00-\x7F]/g, (char) => latinToEnglishMap[char] || '');

    return handle;
};

export const getDisabledFeature = (feature: string): boolean => {
    return (process.env.NEXT_PUBLIC_DISABLE_FEATURES || '').toLowerCase().split(',').includes(feature);
};
...
export function getFilteredContent(
    ...
    const courseContentWithPercentComplete = courseContent.map((content) => {
        if(content.type === 'video' && content.videoFullDuration){
            const videoProgressPercent = content.duration ? (content.duration / content.videoFullDuration) * 100 : 0;
    ...
    if(filter === 'watched'){
        return courseContentWithPercentComplete?.filter((content) => content?.markAsCompleted === true);
    }

    if(filter === 'watching'){
        return courseContentWithPercentComplete?.filter(
            (content) => (content?.markAsCompleted === false && 
                content?.duration !== null && 
                content?.duration !== 0
            ) || (content?.percentComplete && content?.percentComplete > 0 && content?.percentComplete < 100)
        );
    }

    if(filter === 'unwatched'){
        return courseContentWithPercentComplete?.filter((content) => content?.markAsCompleted === false && content?.duration === 0 ) 
    };
