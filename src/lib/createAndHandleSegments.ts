...
export function segmentsHandler(segments: Segment[], player: any){
    ...
    segments?.forEach((each: Segment) => {
        ...
        previewEle.classList.add('timeline-segments-progress');
...
function progressTimeline(segments: Segment[], player: any){
...
function previewViaMouseOverOrMove(e: any){
    ...
    if(rect){
        const percent = Math.min(Math.max(0, e?.x - rect?.width) / rect?.width);
        return percent;
    }
}
...
function setCurrentLabel(
    ...
    if(timelineLabel && rect && timelineContainer){
        ...
        if(Math.max(0, e?.x - rect?.x) < timelineLabel?.offsetWidth / 2){
            ...
            Math.max(0, e?.x - rect?.x) > timelineContainer.offsetWidth - timelineLabel?.offsetWidth / 2
        ){
            timelineLabelWidth = timelineLabel?.offsetWidth - (timelineContainer?.offsetWidth - (e?.x - rect?.x));
        }
...
export function updateTimeline(e: any, player: any, segments: Segment[]){
    ...
    if(e.type === 'timeupdate'){
        if(!isMouseDown){
            progressTimeline(segments, player);
        }
