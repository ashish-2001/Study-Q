...
const generateJWT = async (payload: JWTPayload) => {
    ...
    const jwt = await new SignJWT({
        ...
        jti: randomUUID()
    }).setProtectedHeader({ alg: 'HS256' }).setExpirationTime('365d').sign(jwk);
        ...
        CredentialsProvider({
            ...
            async authorize(credentials: any){
                try{
                    ...
                    if(userDb && userDb.password && (await bcrypt.compare(credentials.password, userDb.password)) && userDb?.appxAuthTToken){
