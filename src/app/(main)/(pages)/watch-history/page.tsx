...
export type TWatchIstory = VideoProgress & {
    content: Content & {
...
function formatWatchHistoryDate(date: Date){
    const now = new Date();
...
function groupByWatchedDate(userVideoProgress: TWatchHistory[]){
...
async function getWatchHistory(){
    ...
    const filterUserVideoProgress: TWatchHistory[] = userVideoProgress.map((VideoProgress) => {
        const filteredCourse = videoProgress?.content?.parent?.courses.filter(
            (course) =>  courses.some(purchaseCourse) => purchaseCourse.id === courses.courseId, 
        )
    );
