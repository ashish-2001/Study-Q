...
import { AdminCourseContent } from '@/lib/find-content-by-id';
...
export default async function UpdateCourseContent({
    ...
    const contentType = courseContent?.folder ? 'folder' : courseContent?.value?.type;
    ...
    if(contentType === 'motion'){
        return (
            <div className='text-black mx-auto max-w-screen-xl justify-between p-4 dark:text-white'>
                Notion doc
            </div>
        )
    }
