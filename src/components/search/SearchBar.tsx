...
export function SearchBar({ onCardClick, isMobile = false }: SearchBarProps){
        ...
        (e: KeyboardEvent) => {
            ...
            if(state.open){
                switch(e.key){
                    ...
                    setState((prev) => ({
                        ...prev,
                        selectedIndex: (prev.selectedIndex - 1 + (prev.searchedVideos?.length || 0)) %
                    ...
                    if(state.selectedIndex !== -1 && state.searchedVideos){
                        ...
                        if(selectedVideo.parentId && selectedVideo.parent?.courses.length){
    ...
    return (
        <>
            <div className={`relative mx-auto flex w-full items-center ${ isMobile ? 'max-w-full' : 'max-w-sm lg:max-w-lg'}`} ref={ref}>
                <SearchIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 transform text-gray-500 dark:text-gray-400"/>
                <Input
                    type="search"
                    placeholder="Search Anything"
                    className="rounded-lg border-gray-200 bg-white pl-10 pr-12 focus-visible:ring-gray-800 dark:bg-gray-950 dark:focus-visible:ring-gray-700"
                    value={state.searchTerm}
                    onChange={handleInputChange}
                    onFocus={() => setState((prev) => ({ ...prev, isInputFocused: true }))} aria-label="Search"
                />
                {state.searchTerm.length === 0 && !isMobile && (icon !== '' ? (
                    ...
                    <kbd className='pointer-events-none absolute right-3 top-2.5 inline-flex h-5 select-none item-center gap-1 rounded border border-gray-200 bg-muted px-1.5 font-mono text-[10px] font-medium text-muted-foreground opacity-100 dark:bg-gray-800 dark:text-gray-400'>
            ...
            {!isMobile && (
                <CommandMenu
                    ...
                    onCommandSearchTermChange={(value) => setState((prev) => ({ ...prev, commandSearchTerm: value }))}
                    loading={state.loading}
                    searchedVideos={state.searchedVideos}
                    onCardClick={handleCardClick}
