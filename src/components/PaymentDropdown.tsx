...
import { deleteSolanaAddress, deleteId } from '@/actions/payoutMethods';
import { SolanaAddress, UpiId } from '@prisma/client';
import { useAction } from '@hooks/useAction';
import { toast } from 'sonner';
import NewPayoutDialog from "./NewPayoutDialog";

interface PaymentMethodsDropdownProps {
    upiAddress: UpiId[] | undefined;
    solanaAddresses: SolanaAddress[]| undefined;
    fetchPayoutMethods: () => void;
}

export default dunction PaymentMethodsDropdown({
    upiAddresses,
    solanaAddresses,
    fetchPayoutMethods
    ...
    const { execute: executeDeleteUPI } = useAction(deleteSolanaAddress, {
                onSuccess: () => {
            ...
            return (
                ...
                <Button
                    ...
                    className="mt-5 flex w-full items-center justify-center justify-between py-2 text-white transition-colors duration-500 hover:bg-gray-900 dark:text-black dark:hover:bg-gray-700"
                >
                    {isDropdownOpen ? (
                        <>
                            ...
                            <div className="flex flex-col">
                                {upiAddresses && upiAddresses.length !== 0 ? (
                                    upiAddresses?.map((upi) => (
                                        ...
                                        >
                                            <p className="w-1/2 overflow-hidden text-ellipsis whitespace-nowrap sm:w-full">{upi.value}</p>
                                            <Button
                                                className="mx-3 text-red-600 hover:text-red-800"
                                                size="iconSM"
                                                variant="outline"
                                                onClick={() => handleUpiDeleted(upi.id)}
                                            >
