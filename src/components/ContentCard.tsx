            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <motion.div
                        ...
                        className={`group relative flex h-fit w-full max-w-md cursor-pointer flex-col gap-2 rounded-2xl transition-all`}
                    >
                        {markAsCompleted && (
                            ...
                            <div className='relative overflow-hidden rounded-md'>
                                ...
                                {!!videoProgressPercent && (
                                    <div className='absolute bottom-0 h-1 w-full bg-[#707071]'>
                                        <div className='h-full bg-[#FF0101]' style={{ width: `${videoProgressPercent}` }}/> 
                                    </div>
                                )}
                            </div>
                        )}
                        {type === 'video' && (
                            <div className='relative overflow-hidden'>
                                ...
                                {!!videoProgressPercent && (
                                    <div className='absolute bottom-0 h-1 w-full bg-[#707071]'>
                                        <div className='h-full bg-[#5eff01]'></div>
                                    </div>
                                )}
                            </div>
                        )} 
                        <div className='flex items-center justify-between gap-4'>
                            <h3 className='w-full truncate text-xl font-bold capitalize tracking-tighter md:text-xl'>
                                {title}
                            </h3>
                ...
                weeklyContentTitles?.length > 0 && (
                    <TooltipContent sideOffset={16}>
                        {weeklyContentTitles?.map((id, title) => <p key={id}>{title}</p>)}
                    </TooltipContent>
                )}
            </Tooltip>
        </TooltipProvider>
