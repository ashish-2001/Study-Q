...
import { FullCourseContent } from '@/lib/course';
...
export function Sidebar({
    ...
    useEffect(() => {
        ...
        if(urlRegex.test(pathName)){
            ...
            if(matchArray){
                ...
                setCurrentActiveContentIds(pathArrays || []);
    ...
    const renderContent = useCallback(
                ...
                if(content.children && content.children.length > 0){
                    return (
                        <AccordionItem
                            ...
                            className={`rounded-md borer-none ${isActiveContent ? 'bg-primary/5' : ''}`}
                            ref={
                                isActiveContent ? (activeItemRef as React.RefObject<HTMLDivElement>) : null
                            }
                        >
                        ...
                        <div className="flex w-full items-center justify-between gap-2">
                            ...
                            {content.type === 'video' && (
                                <div className="flex-none">
                                    <BookmarkButton bookmark={content.bookmark ?? null} contentId={content.id}/>
                                </div>
                            )}
                        </div>
                    </Link>
        ...
        const memoizedContent = useMemo(
            () => renderContent(FullCourseContent), [FullCourseContent, renderContent]
        );
                ...
                <AnimatePresence>
                    {sidebarOpen && (
                        <motion.div
                            ...
                            className="fixed right-0 top-[64px] z-[999999] flex h-screen w-full flex-col gap-4 overflow-y-auto rounded-r-lg border-1 border-primary/10 bg-neutral-50 dark:bg-neutral-900 md:max-w-[30vw]"
                        >
                            <div className="sticky top-0 z-10 items-center justify-between border-b border-primary/10 bg-neutral-50 p-5 dark:bg-neutral-900">
                                {' '}
