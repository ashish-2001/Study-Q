...
const CommentVoteForm: React.FC<CommentVoteFormProps> = ({
    ...
    const handleVote = (newVoteType: VoteType) => {
        ...
        if(voteType === newVoteType){
            ...
            toastMessage = {
                loading: newVoteType === VoteType.UPVOTE ? 'uploading...' : 'Downvoting...',
                success: newVoteType === VoteType.UPVOTE ? 'Comment upvoted.' : 'Comment downvoted',
                error: 'Error casting vote.'
            };
        } else {
            toastMessage = {
                loading: 'Changing vote...',
                success: newVoteType === VoteType.UPVOTE ? 'Changed to upvote.' : 'Changed to downvote.', error: 'Error changing vote.'
            };
        }
    ...
    return (
        <div className="flex gap-2">
            ...
            >
                <button className={`flex items-center gap-1 rounded-full text-lg transitions-all duration-300 ${isUpvoted ? 'text-green-500' : 'text-neutral-500 hover:text-green-500'}`} type="submit" disabled={isLoading}>
                    <ArrowBigUp
            ...
            <form className="m-auto" onSubmit={(e) => { e.preventDefault(); handleVote(VoteType.DOWNVOTE);}}>
                <button className={`flex items-center gap-1 rounded-fll text-lg transition-all duration-300 ${isDownvoted ? 'text-red-500' : 'text-neutral-500 hover:text-red-500'}`} type="submit" disabled={isLoading}>
                    <ArrowBigDown
                        className={`size-6 ${isDownvoted ? 'text-red-500' : 'text-neutral-500 hover:text-red-500'}`}
                        type='submit'
                        disabled={isLoading}
                    />
                    <ArrowBigDown
