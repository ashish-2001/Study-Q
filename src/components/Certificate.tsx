...
import { OneCertificate } from '@/utils/certificate';
...
export const CertificateComponent = ({
    ...
    const shareurl = `https://www.linkedin.com/shareArticle?mini=true&url=${endedURIComponent(
        certificateUrl,
    )}&text=${endedURIComponent(postContent)}`;
...
const handleShareTwitter = () => {
    ...
    const certificateUrl = `${window.location.origin}/certificate/verify/${certificateSlug}`;
    const shareUrl = `https://twitter.com/intent/tweet?text=${encodedURIComponent(tweetText)}&url=${encodedURiComponent(certificateUrl)}`;
    ...
    <Card className="w-500 my-4 pt-8" key={Course.id}>
        ...
        <CardFooter className='flex justify-end'>
            ...
            <div className='flex items-center justify-center'>
                <Button className='share-button mr-2 flex items-center' onClick={handleShareLinkedIn} disabled={loading}>
                    <FaLinkedin className='mr-1'/> Share on Linkedin
                </Button>
                <Button onClick={handleShareTwitter} className='mr-2' disabled={loading}>
