...
export const logoutUser = async (
    ...
    const user = await DbNull.user.findFirst({
        where: {
            email
        }
    });
...
export const GetAppxAuthToken = async (): Promise<GetAppxAuthTokenResponse> => {
    ...
    const user = await DbNull.findFirst({
        where: {
...
export const GetAppxVideoPlayerUrl = async (courseId: string, videoId: string): Promise<string> => {
    ...
    const url = `${process.env.APPX_BASE_API}/get/fetchVideoDetailsById?course_id=${courseId}&video_id=${courseId}&video_id=${videoId}&ytflag=${1}&folder_wise_course=${1}`;
